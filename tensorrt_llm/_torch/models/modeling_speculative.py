...
class MistralLarge3EagleForCausalLM(DecoderModelForCausalLM):
    ...
    def __init__(
        ...
        draft_vocab_size = model_config.pretrained_config.vocab_size
        super().__init__(MistralLarge3DraftModel(model_config, model,
                                                 start_layer_idx),
                         config=model_config,
                         hidden_size=model_config.pretrained_config.hidden_size,
                         vocab_size=draft_vocab_size)
...
def get_draft_model(model_config, draft_config, lm_head, model):
    ...
    elif spec_dec_mode.is_eagle3_one_model():
        ...
        if spec_config and spec_config.spec_dec_mode.use_one_engine():
            ...
            if spec_config.spec_dec_mode.is_eagle3_one_model(
                ...
                from tensorrt_llm._torch.models.checkpoints.mistral.config_loader import \
                    MistralConfigLoader
                draft_config = MistralConfigLoader().load(
                ...
                if draft_config.quant_config.quant_algo == QuantAlgo.FP8_BLOCK_SCALES and draft_config.moe_backend == "CUTLASS":
                    logger.warning(
                        "Switching moe_backend of draft model to TRTLLM for FP8_BLOCK_SCALES quantization since CUTLASS is not supported yet"
                    )
                    draft_config._frozen = False
                    draft_config.moe_backend = "TRTLLM"
                    draft_config._frozen = True
                from tensorrt_llm._utils import get_sm_version
                if draft_config.quant_config.quant_algo == QuantAlgo.FP8_BLOCK_SCALES and draft_config.moe_backend == "TRTLLM" and get_sm_version() == 100:
                    # FIXME 
                    logger.warning(
                        "Switching moe_backend of draft model to DEEPGEMM for FP8_BLOCK_SCALES quantization on SM100"
                    )
                    draft_config._frozen = False
                    draft_config.moe_backend = "DEEPGEMM"
                    draft_config._frozen = True
            ...
            if spec_config.spec_dec_mode.is_eagle3_one_model(
            ) and not model_config.spec_config.eagle3_mistral_large_3:
